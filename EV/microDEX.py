...
# CONTROLS
# ======================================================================
DEV = False  # additional feedback in terminal log
...
def buy(price, amount):
    ...
    account_state()
    edicts = [
        {"op": "buy", "price": price, "amount": amount, "expiration": 0}
    ]
    place_order(edicts)
...
def Bitshares_Trustless_Client():
    ...
    for i in range(5):
        try:
            ...
            msg = (
                str(time.ctime() + " " + type(e).__name__)
                + " "
                + str(e.args)
            )
...
def initialize():
    ...
    print(
        "      Welcome Back: ",
        it("green", account_name),
        it("blue", str(account_id)),
    )
    print("")
...
def tk_master():
    ...
    header_text = tk.Text(
        master, height=5, width=90, fg=blue1, bg=gray3, wrap=tk.NONE
    )
    header_text.insert(tk.END, "")
    header_text.grid(row=0, column=0, columnspan=4)
    bid_text = tk.Text(
        master, height=20, width=45, fg=green1, bg=gray3, wrap=tk.NONE
    )
    ...
    ask_text = tk.Text(
        master, height=20, width=45, fg=red1, bg=gray3, wrap=tk.NONE
    )
    ask_text.insert(tk.END, "")
    ask_text.grid(row=1, column=2, columnspan=2)
    orders_text = tk.Text(
        master, height=11, width=45, fg=yellow1, bg=gray3, wrap=tk.NONE
    )
    orders_text.insert(tk.END, "")
    orders_text.grid(row=2, column=0, columnspan=2)
    history_text = tk.Text(
        master, height=11, width=45, fg=gray1, bg=gray3, wrap=tk.NONE
    )
    history_text.insert(tk.END, "")
    history_text.grid(row=2, column=2, columnspan=2)
    account_text = tk.Text(
        master, height=4, width=90, fg=blue1, bg=gray3, wrap=tk.NONE
    )
    account_text.insert(tk.END, "")
    account_text.grid(row=3, columnspan=4)
    tk.Label(master, text="PRICE:", fg=gray1, bg=gray3).grid(
        row=4, column=0, sticky=tk.E
    )
    tk.Label(master, text="AMOUNT:", fg=gray1, bg=gray3).grid(
        row=5, column=0, sticky=tk.E
    )
    tk.Label(master, text="PRICE:", fg=gray1, bg=gray3).grid(
        row=4, column=2, sticky=tk.E
    )
    tk.Label(master, text="AMOUNT:", fg=gray1, bg=gray3).grid(
        row=5, column=2, sticky=tk.E
    )
    tk.Label(master, text="litepresence2019", fg=gray2, bg=gray3).grid(
        row=9, column=3, sticky=tk.SE
    )
    lock_label = tk.StringVar()
    lock_label.set("")
    tk.Label(
        master, textvariable=lock_label, fg=yellow1, bg=gray3
    ).grid(row=8, column=2, sticky=tk.W)
    tk.Label(
        master,
        text=(
            "   *ORDERS TAKE A FEW SECONDS TO APPEAR; "
    ...
    try:
        try:
            ...
            download_img(
                "https://i.imgur.com/fvCE86Z.gif", "bitshares_logo.gif"
            )
            photo = tk.PhotoImage(file="bitshares_logo.gif")
    ...
    tk.Button(
        ...
        highlightbackground=green1,
    ).grid(row=7, column=1, pady=4)
    tk.Button(
        master,
        text="SELL",
        command=tk_sell,
        ...
        highlightbackground=red1,
    ).grid(row=7, column=3, pady=4)
    tk.Button(
        master,
        text="CANCEL ALL",
        command=tk_cancel,
        ...
        highlightbackground=yellow1,
    ).grid(row=7, column=0, sticky=tk.E, pady=4)
    tk.Button(
        master,
        text="LOCK/UNLOCK",
        ...
        highlightbackground=blue1,
    ).grid(row=8, column=0, sticky=tk.E, pady=4)
...
def tk_animate():
    ...
    global lock_label, runtime, read_time, pause, pauses
            ...
            try:
                ...
                for i in range(len(orders)):
                    orders[i] = {
                        ...
                        if k in ["price", "orderType", "amount"]
                    }
                ...
                # stringified volume
                sbidp = [
                    (" %.16f " % i).rjust(20, " ") for i in book["bidp"]
                ]
                saskp = [
                    (" %.16f " % i).ljust(20, " ") for i in book["askp"]
                ]
                sbidv = [
                    (" %.2f " % i).rjust(12, " ") for i in book["bidv"]
                ]
                saskv = [
                    (" %.2f " % i).rjust(12, " ") for i in book["askv"]
                ]
    ...
    header_text.insert(
        tk.END,
        (
            ("   UI %s" % ("%.1f" % (ui_refresh * 1000)).rjust(5))
            + ("  PAUSE %s" % ("%.1f" % (pause)).rjust(5))
            + ("  PING %.3f" % ping)
            + ("   READ %.5f" % read_time)
    ...
    if update_id % DATA_MODULUS == 0:
        ...
        for i in range(len(saskp)):
            ask_text.insert(
                tk.END, (saskp[i] + saskv[i] + caskv[i] + "\n")
            )
            bid_text.insert(
                tk.END, (cbidv[i] + sbidv[i] + sbidp[i] + "\n"), "right"
            )
    ...
    p = int(
        ...
        * min(
            UI_REFRESH,
            (max(0.001, (2 * UI_REFRESH - ui_refresh + ui_delta))),
        )
    )
    pauses.append(p)
    pauses = pauses[-20:]
    pause = sum(pauses) / float(len(pauses))
    master.after(p, tk_animate)
...
def tk_buy():
    ...
    if authenticated:
        ...
        try:
            ...
            if currency == "BTS":
                currency_balance -= 2
            # warn and scale on extreme amount
            means = SIXSIG * currency_balance / decimal(price)
            if amount > means:
                amount = means
                warn = tk.Tk()
                warn.title("WARN: BUY AMOUNT TOO BIG SCALING TO MEANS")
                warn.geometry("500x100")
                button = tk.Button(
                    warn,
                    text="PROCEED TO CONFIRM / INVALIDATE WINDOW",
                    command=warn.destroy,
                )
                button.pack()
            # warn on extreme price
            price_over = 100 * (float(price) / float(last)) - 100
            if price_over > 3:
                warn_text = (
                    "WARN: BUY PRICE %.1f PERCENT OVER LAST"
                    % price_over
                )
                warn = tk.Tk()
                warn.title(warn_text)
                warn.geometry("500x100")
                button = tk.Button(
                    warn,
                    text="PROCEED TO CONFIRM / INVALIDATE WINDOW",
            ...
            sorder = str(
                "CONFIRM BUY " + samount + " " + asset + " @ " + sprice
            )
            if amount > 0:
            ...
            tk.Button(confirm, text="OK", command=confirm.destroy).grid(
...
def tk_sell():
    ...
    if authenticated:
        ...
        try:
            ...
            price = decimal(price)
            amount = decimal(amount)
            # warn and scale extreme amount
            if amount > (SIXSIG * asset_balance):
                amount = SIXSIG * asset_balance
                warn = tk.Tk()
                warn.title("WARN: SELL AMOUNT TOO BIG SCALING TO MEANS")
                warn.geometry("500x100")
                button = tk.Button(
                    warn,
                    text="PROCEED TO CONFIRM / INVALIDATE WINDOW",
                ...
                button.pack()
            # warn on extreme price
            price_under = 100 - 100 * (float(price) / float(last))
            if price_under > 3:
                warn_text = (
                    "WARN: SELL PRICE %.1f PERCENT UNDER LAST"
                    % price_under
                )
                warn = tk.Tk()
                warn.title(warn_text)
            ...
            samount = str(amount)[:16]
            sorder = str(
                "CONFIRM SELL " + samount + " " + asset + " @ " + sprice
            )
            if amount > 0:
                confirm.title(sorder)
                ...
                tk.Button(
                    confirm,
                    text="CONFIRM SELL",
                    command=lambda: sell(price, amount),
                ).grid(row=1, column=0, pady=8)
                tk.Button(
                    confirm,
                    text="INVALIDATE",
...
def tk_cancel():
    ...
    if len(orders):
        if authenticated:
            ...
            tk.Button(
                confirm, text="CONFIRM CANCEL ALL", command=cancel
            ).grid(  # lambda: cancel(market)).grid(
