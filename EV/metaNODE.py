    ...
    while True:
        try:
            ...
            if any([x in str(e.args) for x in race_condition]):
                print(
                    "metaNODE = Bitshares_Trustless_Client() RACE READ"
                )
...
def rpc_ping_latency():
    start = time.time()
    chain_id = wss_query(["database", "get_chain_id", []])
...
def rpc_block_latency():
    dgp = wss_query(["database", "get_dynamic_global_properties", []])
    blocktime = from_iso_date(dgp["time"])
    block_latency = min(9.999, (time.time() - blocktime))
...
def rpc_last(currency, asset):
    ticker = wss_query(
        ["database", "get_ticker", [currency, asset, False]]
    )
...
def rpc_account_balances(
    account_name, asset_ids=[], asset_precisions=[]
):
    ...
    for j in range(len(asset_ids)):
        for k in range(len(ret)):
            if ret[k]["asset_id"] == asset_ids[j]:
                balances[asset_ids[j]] += (
                    float(ret[k]["amount"]) / 10 ** asset_precisions[j]
                )
    return balances
...
def rpc_book(currency, asset, depth=3):
    order_book = wss_query(
        ["database", "get_order_book", [currency, asset, depth]]
    )
    askp = []
    bidp = []
    askv = []
    ...
    while True:
        ...
        for a in range(PROCESSES):
            ...
            try:
                ...
                pass
            try:
                multinode[str(a)] = Process(
                    target=thresh, args=(a, b, c)
                )
                multinode[str(a)].daemon = False
                multinode[str(a)].start()
            except Exception as e:
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                data_latency = time.time() - blocktime
                del metaNODE
            ...
            for i in range(utilizations):
                ...
                block_latency, block_max, blocktime = (
                    rpc_block_latency()
                )
                set_timing = (
                ...
                try:
                    ...
                    cpu = "%.3f" % (float(os.popen(usage).readline()))
                ...
                if (len(white) < bw_depth) or (len(black) < bw_depth):
                    alert = it(
                        "red", "    BUILDING BLACK AND WHITE LISTS"
                    )
                else:
                    alert = ""
                alert += static
                ...
                for i in range(3):
                    print(
                        it("red", precision(m_askp[i], 16)), m_askv[i]
                    )
                print("bids depth       ", len(m_bidp))
                for i in range(3):
                    print(
                        it("green", precision(m_bidp[i], 16)), m_bidv[i]
                    )
                print("")
            ...
            try:
                ...
                if (
                    ...
                    and ("SSL" not in msg)
                    and ("WebSocketTimeoutException" not in msg)
...
def bifurcation():  # statistically curate data
    ...
    while True:
        try:
            ...
            ping = (
                int(1000 * sum(pings) / (len(pings) + SATOSHI)) / 1000.0
            )
            ping = min(1, ping)
            ...
            try:
                ...
                try:
                    currency_balance = mode(
                        currency_balance[-(l - 1) :]
                    )
                except:
                    currency_balance = mode(
                        currency_balance[-(l - 2) :]
                    )
            try:
            ...
            currency_holding = float(
                precision(currency_holding, currency_precision)
            )
            asset_holding = float(
                precision(asset_holding, asset_precision)
            )
            currency_max = float(
                precision(currency_max, currency_precision)
            )
            ...
            metaNODE["currency_balance"] = float(
                currency_balance
            )  # FLOAT
    ...
    while True:
        ...
        if metaNODE:
            try:
                ...
                race_append(
                    doc="account_history.txt", text=json_dumps(snapshot)
                )
                time.sleep(3600)
            except Exception as e:
                msg = trace(e)
