...
sys.stdout.write(
    "\x1b]2;" + "Bitshares accountHISTORY" + "\x07"
)  # terminal #title
    ...
    while not opened:
        try:
            with open(doc, "r") as f:
                ...
                # print(ret)
                ret = [
                    json.loads(str(r).replace("'", '"')) for r in ret
                ]
                opened = 1
...
def draw_chart():
    ...
    plt.title(("price " + asset + ":" + currency), color="g")
    plt.plot(unix, price, "g.-")
    plt_format()

    plt.subplot(grid[1, 0])
    plt.title(("maximum " + currency), color="r")
    plt.plot(unix, currency_max, "r.-")
    plt_format()

    plt.subplot(grid[1, 1])
    plt.title(("maximum " + asset), color="b")
    plt.plot(unix, asset_max, "b.-")
    plt_format()

    plt.subplot(grid[2, 0])
    plt.title("percent invested", color="g")
    plt.plot(unix, invested, "g.-")
    plt_format()

    plt.subplot(grid[2, 1])
    plt.title("RETURN ON INVESTMENT", color="g")
    plt.plot(unix, currency_roi, "r.-")
    plt.plot(unix, asset_roi, "b.-")
    plt.plot(unix, gross_roi, "g.-")
    plt_format()
    plt.show()
...
def data_set():
    ...
    for i in range(len(doc)):
        ...
        if (doc[i]["unix"] > start) and (doc[i]["unix"] < stop):

            currency_max.append(doc[i]["currency_max"])
            asset_max.append(doc[i]["asset_max"])
            unix.append(doc[i]["unix"])
            invested.append(doc[i]["invested"])
...
global currency, asset
currency = doc[0]["currency"]
asset = doc[0]["asset"]

keys = ["unix", "asset_max", "invested", "currency_max"]
doc = [{k: v for k, v in i.items() if k in keys} for i in doc]
...
f3.pack()
BEGIN = Scale(
    f1, from_=begin, to=end, resolution=1, orient=HORIZONTAL, length=300
)
...
END = Scale(
    f2, from_=begin, to=end, resolution=1, orient=HORIZONTAL, length=300
)
