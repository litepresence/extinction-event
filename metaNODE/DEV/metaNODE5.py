...
def controls():
    ...
    global BOOK_DEPTH, HISTORY_DEPTH
...
def version():
    ...
    VERSION = 'metaNODE v0.00000006 - Bitshares Trustless Client'
...
def cache():
    ...
    # given account name, currency and asset symbols, create these globals
    ...
    # trustless mode statistic of 5 nodes
    for i in range(5):
        wwc()
...
def spawn():  # multiprocessing watchdog
    ...
    # kill and respawn threshing processes periodically
    while 1:
        b += 1
        for a in range(PROCESSES):
            c += 1
            ...
            for i in range(randint(8, 12)):
                ...
                for i in range(len(named_account_balances)):
                    if named_account_balances[i]['asset_id'] == asset_id:
                        asset_balance += float(
                            named_account_balances[i]['amount']) / 10 ** asset_precision
                    elif named_account_balances[i]['asset_id'] == currency_id:
                        currency_balance += float(
                            named_account_balances[i]['amount']) / 10 ** currency_precision
                ...
                print('white : black  ', len(white), ':', len(black))
                print('')
...
def banner():
    ...
    time.sleep(0.5)
    print(
        "    metaNODE['blocktime'] # oldest blockchain time in metaNODE data \n")
    time.sleep(2)
